...
app = typer.Typer(name="synthlog", help="Synthetic Okta System Log generator")
console = Console()
...
def generate(
    ...
    """Generate synthetic Okta System Log events."""
    rng = Random(seed)
    ...
    if verbose:
        ...
        if scenario:
            console.print(f"  scenarios: {', '.join(scenario)}")
...
def serve(
    ...
    """Start mock Okta API server (GET /api/v1/logs)."""
    console.print(f"Starting mock Okta API on port {port} (seed={seed})")
...
def list_events() -> None:
    ...
    for name, meta in sorted(EVENT_TYPE_REGISTRY.items()):
        console.print(f"  {name:40s} {meta.severity:5s}  {meta.display_message}")
